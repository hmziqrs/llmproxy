...
fn parse_serve_with_config() {
    ...
    let Commands::Serve { config, .. } = cli.command else { panic!("expected serve") };
...
fn parse_serve_with_port() {
    ...
    let Commands::Serve { port, .. } = cli.command else { panic!("expected serve") };
...
fn parse_serve_background_flag() {
    ...
    let Commands::Serve { background, .. } = cli.command else { panic!("expected serve") };
    assert!(background);
}
...
fn parse_serve_daemonize_hidden_flag() {
    ...
    let Commands::Serve { daemonize, .. } = cli.command else { panic!("expected serve") };
    assert!(daemonize);
}
...
fn parse_validate_with_config() {
    ...
    let Commands::Validate { config } = cli.command else { panic!("expected validate") };
...
fn parse_models_with_all_flags() {
    ...
    let Commands::Models { provider, live, write_catalog, require_success, .. } = cli.command else {
...
fn parse_models_write_catalog_requires_live() {
    ...
    let result = Cli::try_parse_from([
        "llm-proxy", "models", "--write-catalog",
    ]);
...
fn parse_models_require_success_requires_live() {
    let result = Cli::try_parse_from([
        "llm-proxy", "models", "--require-success",
    ]);
    assert!(result.is_err());
}
